// src/components/Profiles/InstructorProfile.jsx
...
/**
 * Instructor Profile (EN) — Backend-ready
 * - يجلب بيانات المستخدم من /api/profile/:id عند التحميل
 * - يحدث الاسم/الإيميل عبر PUT لنفس المسار
 * - Authorization من localStorage.token
 * - Avatar ثابت "INS" دائمًا
 */
...
export default function InstructorProfile() {
  ...
  React.useEffect(() => {
    async function fetchProfile() {
      try {
        ...
        setUser({
          name: fullName,
          email: data.email || "instructor@example.com",
          role: data.role || "Instructor",
          department: data?.department?.name || "Department of Computer Science",
        });
  ...
  async function onSave(e){
    ...
    try {
      ...
      // نفصل الاسم لأول/أخير لو باك-إندك يعتمدها
      ...
      // تحديث بسيط للـ localStorage (اختياري)
      try {
  ...
  return (
    <div className="ins-prof" dir="ltr">
      {/* Hero */}
      <section className="hero area-constrained">
        ...
        <svg className="wave" viewBox="0 0 1440 120" preserveAspectRatio="none" aria-hidden="true">
          <defs>
            <linearGradient id="insWave" x1="0%" y1="0%" x2="100%" y2="0%">
              <stop offset="0%" stopColor="#c7d2fe" />
              <stop offset="55%" stopColor="#93c5fd" />
              <stop offset="100%" stopColor="#3b82f6" />
            </linearGradient>
          </defs>
          <path d="M0,64 C240,100 480,40 720,64 C960,88 1200,76 1440,60 L1440,160 L0,160 Z" fill="url(#insWave)"/>
        </svg>
      </section>
      ...
      <style>{`
        ...
        .ins-prof{ color:var(--ink); min-height:100vh; display:flex; flex-direction:column; }
