...
// (optional) map any DB-cased fields to UI-cased fields if needed
function mapRow(r) {
  return {
    ...
    // sender/receiver info populated by server join when available
...
// Turn raw notification + data into a short human-friendly sentence
function buildMessage(n) {
  try {
    const d = n.data || {};
    if ((n.type === "request" || n.entity === "CommitteeRequest") && (d.action === "registrar_response" || d.responseType || d.category)) {
      const who = n.sender_name ? n.sender_name : (n.sender_role ? `${n.sender_role}` : "Registrar");
      ...
      const irregular = String(d.category || d.responseType || "").toLowerCase().includes("irregular");
      let msg = `${who} ${verb} ${irregular ? "an irregular update for" : "a request for"} ${student}`;
      const parts = [];
      ...
      if (d.replace !== undefined || d.Replace !== undefined) {
        const rep = d.replace ?? d.Replace;
        parts.push(rep ? "Replaced previous courses" : "Appended to existing courses");
      }
      if (parts.length) msg += ` (${parts.join("; ")})`;
      return msg;
    }

    if ((n.type === "request" || n.entity === "CommitteeRequest") && (d.action === "request_status" || n.message?.toLowerCase().includes("updated status"))) {
...
export default function Notification() {
  ...
  const load = useCallback(async () => {
    try {
      const url = `${API_BASE}/Notifications/view?limit=20${
  ...
  const markRead = async (id) => {
    try {
      await axios.patch(
        ...
        { receiverId }, // backend expects body receiverId
        auth
      );
